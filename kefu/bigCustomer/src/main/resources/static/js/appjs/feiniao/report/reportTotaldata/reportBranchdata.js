...
//								dateStyle:$("#dateStyle").find("option:selected").attr("value"),
//								qu_date:$('#qu_date').val(),
...
//							    region_id : "北京大区",
//							    city_id : "120100",
//								dateStyle:$("#dateStyle")val(),
//								qu_date:$('#quDate').val(),
//								month_year:$('#monthYear').val(),
//								quarter_year:$('#quarterYear').val(),
//								quarter_date:$('#quarterDate').val(),
//								year:$('#year').val(),
								start_date:$('#startDate').val(),
								end_date:$('#endDate').val(),
								province_id :$('#provinceid').val(),
							    region_id : $('#bigarea').val(),
							    city_id : $('#cityid').val(),
							    showType : "branch"
							        		   		  ...
							        		   		  {field : 'customerName', title :  '地域', align : 'center', rowspan : 2, width : 180,
							        		  			formatter:function(value,row, index){
															if(row.customerName=='no'||row.customerName=='wait'||row.customerName=='rqcw' ||row.customerName=='bt')
										                	  return "--";
															 else if(row.customerName ==null || row.customerName ==''  || row.customerName.indexOf("合计")!=-1)
	              							                	  return value;
	             							                		  else
	             							                			  {
	             							                			  if(row.tmpField=='D'||row.tmpField=='S')
	             							                				  return value;
	             							                			  else
	             							                				  return "<a href='javascript:void(0);' style='color:blue;' onclick='openCustomerTable(\""+row.tmpField+","+row.branchCode+","+row.startDate+","+row.endDate+"\");'>"+value+'</a>';  
	             							                			  }
														}
							        				  },
...
//		var year  = $('#year').val();
		var start_date  =param.split(',')[2];
		var end_date  = param.split(',')[3];
		var tmpField  = param.split(',')[0];
