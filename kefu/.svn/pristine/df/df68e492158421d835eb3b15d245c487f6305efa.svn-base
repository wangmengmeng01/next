var prefix = "/vipkf/system/provinceBigarea"
$(function() {
	load();
...
function add() {
	layer.open({
		...
		area : [ '800px', '520px' ],
		content : prefix + '/addProvince' // iframe的url
	});
}
function edit(id) {
	if(s_edit_h=='hidden'){
	...
	layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '520px' ],
		content : prefix + '/editProvinceBigarea/' + id // iframe的url
	});
}
function remove(id) {
	layer.confirm('确定要删除选中的记录？', {
		...
		$.ajax({
			url : prefix+"/removeProvinceBigarea",
			type : "post",
			data : {
				'provinceid' : id
			},
