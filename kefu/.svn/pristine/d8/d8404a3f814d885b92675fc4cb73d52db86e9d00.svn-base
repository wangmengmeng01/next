...
$().ready(function() {
    ...
    $.ajax({
        type:"POST",
        url:prefix+"/statementType/jiedanList",//+consultType,
        data : $('#signupForm').serialize(),// 你的formid
        dataType: "json",
        success:function(res){
            // console.log(res);
            for(var key in res){
                $("#jieDanResult").append("<option value='" + res[key] + "'>" + res[key] + "</option>");
            }
        }
    });
...
function save() {
    ...
    $.ajax({
        cache : true,
        type : "POST",
        url : prefix +"/order/piLiangJieDanSave",
        data : jieDanData,
        async : false,
        ...
        success : function(res) {
            if (res.code == 0) {
                ...
                parent.reLoad();
                //改变父页面的数据
                parent.$('#detail_state').val("已结单");
                parent.$('#detail_state').text("已结单");
                parent.$('#btnDiv').hide();
