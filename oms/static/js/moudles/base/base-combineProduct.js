$(function() {
	$("#dg").datagrid({
		loadMsg : '正在加载中',
		title : '组合商品列表',
		url : './index.php?r=base/combineProduct/data',
		method : 'POST',
		rownumbers : true,
...
function openDeatilView() {
	...
	addTab('tt', '组合商品明细报表', deatilViewUrl.replace('uid', rowData.combine_id).replace('uname', rowData.item_code), '');
}
...
function searchForm() {
	$("#dg").datagrid({
		queryParams : {
			'CombineProduct[item_code]' : $("#itemCode").val(),
			'CombineProduct[customer_id]' : $("#customerId").val(),
			'CombineProduct[warehouse_code]' : $("#warehouseCode").val()
		}
	})
}
...
function openExportExcel() {
	...
	if (itemCode == '' && customerId == '' && warehouseCode == '') {
		$.messager.show({
			title : '友情提示',
			msg : '商品编码、货主ID和仓库编码查询条件不能都为空'
		});
		return false;
	}
	window.location.href = 'index.php?r=export/index&exportType=combineProduct&itemCode='
			+ itemCode
			+ '&customerId='
			+ customerId
			+ '&warehouseCode='
