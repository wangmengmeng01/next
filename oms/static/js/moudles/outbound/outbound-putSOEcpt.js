$(function() {
	$("#dg").datagrid({
		loadMsg : '正在加载中',
		title : '出库单异常列表',
		url : './index.php?r=outbound/putSOEcpt/data',
		method : 'POST',
		rownumbers : true,
...
function openDeatilView() {
	...
	addTab('tt', '异常出库单明细报表', deatilViewUrl.replace('uid', rowData.order_id).replace('uname', rowData.order_no), '');
}
...
function searchForm() {
	$("#dg").datagrid({
		queryParams : {
			'PutSOEcpt[order_no]' : $("#orderNo").val(),
			'PutSOEcpt[order_type]' : $("#orderType").combobox('getValue'),
			'PutSOEcpt[customer_id]' : $("#customerId").val(),
			'PutSOEcpt[warehouse_code]' : $("#warehouseCode").val(),
			'PutSOEcpt[start_time]' : $("#startTime").datetimebox("getValue"),
			'PutSOEcpt[end_time]' : $("#endTime").datetimebox("getValue")
		}
	})
}
...
function openExportExcel() {
	...
	window.location.href = 'index.php?r=export/index&exportType=putSoEcpt&orderNo='
			+ orderNo
			+ '&orderType='
			+ orderType
			+ '&customerId='
