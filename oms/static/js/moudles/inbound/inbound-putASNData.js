$(function() {
	$("#dg").datagrid({
		loadMsg : '正在加载中',
		title : '入库单下发列表',
		url : './index.php?r=inbound/putASNData/data',
		method : 'POST',
		rownumbers : true,
...
function openDeatilView() {
	...
	addTab('tt', '入库单明细报表', deatilViewUrl.replace('uid', rowData.order_id)
			.replace('uname', rowData.order_no), '');
}
...
function searchForm() {
	$("#dg").datagrid({
		queryParams : {
			'PutASNData[order_no]' : $("#orderNo").val(),
			'PutASNData[order_type]' : $("#orderType").combobox('getValue'),
			'PutASNData[customer_id]' : $("#customerId").val(),
			'PutASNData[warehouse_code]' : $("#warehouseCode").val(),
			'PutASNData[start_time]' : $("#startTime").datetimebox("getValue"),
			'PutASNData[end_time]' : $("#endTime").datetimebox("getValue")
		}
	})
}
...
function openExportExcel() {
	...
	window.location.href = 'index.php?r=export/index&exportType=asnData&orderNo='
			+ orderNo
			+ '&orderType='
			+ orderType
			+ '&customerId='
