...
class OrderSequenceServiceClient{
	...
	public static function getService(){
		...
		if (! OrderSequenceServiceClient::$isRegis){
			
			OrderSequenceServiceClient::regis ();
			OrderSequenceServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = OrderSequenceServiceClient::$DEFAULT_PROXY_IP;
		$port = OrderSequenceServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \com\vip\vop\vcloud\order\OrderSequenceServiceHelper();
	}
