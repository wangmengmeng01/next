 ...
 *
 */
require API_ROOT . '/router/interface/wms/qimen/assign/wmsRequest.php';
...
class wmsOrderCancel extends wmsRequest
{
        ...
        if (empty($params)) {
            ...
            if ($params['ownerCode'] == 'XZW') {
                ...
                if (!empty($shortOrders)) {
                    ...
                    //当有一个取消成功的订单，就需要更新状态
                    if ($cancelStatus == 1) {
                        if ($params['cancelReason'] == 'YDCPOMS') {//oms转仓取消
                            foreach ($shortOrders as $s_v) {
                                //更新缺货订单列表订单状态
                                $this->updateShortOrderStatus($s_v['order_id'],3);
                            }
                        } else {
                            foreach ($shortOrders as $s_v) {
                                //更新缺货订单列表订单状态
                                $this->updateShortOrderStatus($s_v['order_id'],4);
                            }
                        }
                    }
    ...
    /**
     * 更改缺货列表订单状态:3 已取消【oms】;4 已取消【erp】
     * @param $params
     */
    public function updateShortOrderStatus($orderId,$status){
        global $db;
        $sql = "UPDATE t_delivery_order_shortage 
                SET done_flag=:done_flag 
                WHERE order_id=:order_id;";
        $model = $db->prepare($sql);
        $model->bindParam(':done_flag', $status);
