...
class CloudToolsServiceClient{
	...
	public static function getService(){
		...
		if (! CloudToolsServiceClient::$isRegis){
			
			CloudToolsServiceClient::regis ();
			CloudToolsServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = CloudToolsServiceClient::$DEFAULT_PROXY_IP;
		$port = CloudToolsServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \com\vip\isv\tools\CloudToolsServiceHelper();
	}
