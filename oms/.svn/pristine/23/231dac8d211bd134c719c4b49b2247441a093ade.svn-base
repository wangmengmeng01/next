...
class wmsDeliveryOrderCreate extends wmsRequest
{
	...
	public function insertDeliveryOrder($params)
	{
		...
		//手机商城订单标记
        $values[':fx_flag'] = empty($params['deliveryOrder']['custmSource']) && !isset($params['deliveryOrder']['custmSource']) ? 0 :1;
        $values[':depositPay'] = isset($params['deliveryOrder']['depositPay']) && $params['deliveryOrder']['depositPay']=='Y' ? 1 :0;
