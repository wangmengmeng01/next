...
class OspStub {
	...
	public function initInvocation($method) {
		...
		// 设置客户端IP
		$ip = \Osp\Util\IPUtil::getIp ();
		$ctx->setUip ( \Osp\Util\IPUtil::IPV42Int32 ( $ip ) );
		\Osp\Log\Logger::debug("[OspStub][InitInvocation][ClientIP][".$ip."]");
	}
	...
	public function send_base($args) {
		...
		$client = new \Osp\Http\HttpClient($url["scheme"], $url["host"], $url["port"]);
		$request = array();
