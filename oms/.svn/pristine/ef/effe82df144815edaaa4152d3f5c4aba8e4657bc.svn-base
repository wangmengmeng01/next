...
class VipVopBiSyncServiceClient{
	...
	public static function getService(){
		...
		if (! VipVopBiSyncServiceClient::$isRegis){
			
			VipVopBiSyncServiceClient::regis ();
			VipVopBiSyncServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = VipVopBiSyncServiceClient::$DEFAULT_PROXY_IP;
		$port = VipVopBiSyncServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \com\vip\isv\sync\VipVopBiSyncServiceHelper();
	}
