...
class wmsDeliveryOrderCreate extends wmsRequest
{
	...
	public function checkDeliveryOrder($params)
	{
		...
		$model->bindValue(':order_no', $params['deliveryOrder']['deliveryOrderCode']);
		$model->bindValue(':order_type', $params['deliveryOrder']['orderType']);
		$model->bindValue(':customer_id', empty($params['orderLines']['orderLine']['ownerCode']) ? (!empty($params['orderLines']['orderLine'][0]['ownerCode']) ? : qimen_service::$_customerId) : $params['orderLines']['orderLine']['ownerCode']);
		$model->bindValue(':warehouse_code', $params['deliveryOrder']['warehouseCode']);
		$rs = $model->queryRow();
		if (!empty($rs)) {			
			//更新原发货单有效性
	...
	public function updateDeliveryOrder($deliveryId)
	{
		...
		$model = $db->createCommand($sql);
		$model->bindValue(':delivery_id', $deliveryId);
		$model->execute();
        return true;
	}
	...
	public function insertDeliveryOrder($params)
	{
		...
		$db = Yii::app()->db;
		$model = $db->createCommand($orderInfoSql);
		$values = array();		
		...
		if (!empty($params['orderLines']['orderLine'])) {
			...
			$model = $db->createCommand($orderDetailSql);			
		...
		if (!empty($params['deliveryOrder']['invoices']['invoice'])) {
			...
			$model = $db->createCommand($orderBillSql);
			foreach ($params['deliveryOrder']['invoices']['invoice'] as $a => $b)
			{
				...
				if (!empty($b['detail']['items']['item'])) {
					...
					$model = $db->createCommand($orderBillProductSql);
