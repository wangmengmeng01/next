...
class wmsReturnOrderCreate extends wmsRequest
{
    ...
    public function insert_return_order($params){
        ...
        $customerId = empty($params['orderLines']['orderLine'][0]['ownerCode']) ? (empty($params['orderLines']['orderLine']['ownerCode']) ? '': $params['orderLines']['orderLine']['ownerCode']): $params['orderLines']['orderLine'][0]['ownerCode'];
    ...
    public function has_upd_return_order($orderNo, $orderType,$customerId,$warehouseCode)
    {
        ...
        $Ssql = 'SELECT COUNT(*) t, order_id FROM `t_inbound_info` WHERE order_no = :order_no AND customer_id = :customer_id AND warehouse_code = :warehouse_code AND is_valid = 1;';
        ...
        if ($rs['t'] > 0) {
            ...
            $Usql = ' UPDATE `t_inbound_info` SET is_valid = 0 WHERE order_no = :order_no AND customer_id = :customer_id AND warehouse_code=:warehouse_code; ';
    ...
    public function updateOrderNoTypeRelation($params){
        ...
        $model = $db->prepare($sql);
        $model->bindParam(':order_no',$params['order_no']);
        $model->bindParam(':order_type',$params['order_type']);
        $model->bindParam(':customer_id',$params['customer_code']);
        $model->bindParam(':warehouse_code',$params['warehouse_code']);
        $model->execute();
    }
