$(function() {
	$("#dg").datagrid({
		loadMsg : '正在加载中',
		title : '出库单下发发票',
		url : './index.php?r=outbound/deliveryOrderInvoice/data&delivery_id=' + deliveryId,
		method : 'POST',
		rownumbers : true,
...
function openDeatilView() {
	...
	addTab('tt', '发货单发票明细报表', deatilViewUrl.replace('uid', rowData.bill_id), '');
}
...
function openExportExcel(deliveryId, deliveryOrderCode) {
	...
	window.location.href = 'index.php?r=export/index&exportType=deliveryOrderInvoice&delivery_id=' + deliveryId + '&delivery_order_code=' + deliveryOrderCode;
}
