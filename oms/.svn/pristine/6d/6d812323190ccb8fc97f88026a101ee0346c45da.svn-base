...
class VendorStoreEditServiceClient{
	...
	public static function getService(){
		...
		if (! VendorStoreEditServiceClient::$isRegis){
			
			VendorStoreEditServiceClient::regis ();
			VendorStoreEditServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = VendorStoreEditServiceClient::$DEFAULT_PROXY_IP;
		$port = VendorStoreEditServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \com\vip\vop\omni\store\VendorStoreEditServiceHelper();
	}
