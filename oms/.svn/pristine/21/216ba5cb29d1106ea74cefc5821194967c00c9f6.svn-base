...
class ProductPublishServiceForVopClient{
	...
	public static function getService(){
		...
		if (! ProductPublishServiceForVopClient::$isRegis){
			
			ProductPublishServiceForVopClient::regis ();
			ProductPublishServiceForVopClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = ProductPublishServiceForVopClient::$DEFAULT_PROXY_IP;
		$port = ProductPublishServiceForVopClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \com\vip\product\publish\service\ProductPublishServiceForVopHelper();
	}
