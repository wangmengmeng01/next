$(function() {
	$("#dg").datagrid({
		loadMsg : '正在加载中',
		title : '发货单回传包裹',
		url : './index.php?r=outbound/deliveryRecordPackage/data&delivery_id=' + deliveryId,
		method : 'POST',
		rownumbers : true,
...
function openMaterialView() {
	...
	addTab('tt', '发货单回传包裹材料明细报表', materialViewUrl.replace('uid', rowData.package_id), '');
}

//查看商品明细
function openProductView() {
	var rowData = $("#dg").datagrid('getSelected');
	addTab('tt', '发货单回传包裹商品明细报表', productViewUrl.replace('uid', rowData.package_id), '');
}
...
function openExportExcel(deliveryId, deliveryOrderCode) {
	...
	window.location.href = 'index.php?r=export/index&exportType=deliveryRecordPackage&delivery_id=' + deliveryId + '&delivery_order_code=' + deliveryOrderCode;
}
