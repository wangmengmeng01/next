		...
		if (empty($params)) {
			...
			if (!empty($response)) {
				if ($response['flag'] == 'success') {
				    foreach ($params['orders']['order'] as $k => $v) {
				        //校验发货单是否存在，如果存在则状态置为无效
    					$this->checkDeliveryOrder($v);
    					//写入数据库
    					$this->insertDeliveryOrder($v);
				    }
					//返回
				    ...
				    foreach ($params['orders']['order'] as $k => $v) {
				        ...
				        foreach ($returnMsgArr['orders']['order'] as $val) {
				            if ($v['deliveryOrder']['deliveryOrderCode'] == $val['deliveryOrderCode']) {
				                unset($params['orders']['order'][$k]);
				            }
				        }
				    }
				    foreach ($params['orders']['order'] as $k => $v) {
