...
class GetOrderOpStatusReq {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['opList'])){
				
				$this->opList = $vals['opList'];
			}
	...
	public function write($output){
		...
		if($this->opList !== null) {
			...
			$xfer += $output->writeFieldBegin('opList');
			
			if (!is_array($this->opList)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->opList as $iter0){
