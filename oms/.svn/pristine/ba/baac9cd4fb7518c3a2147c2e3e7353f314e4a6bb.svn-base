...
class JitDeliveryInfoServiceClient{
	...
	public static function getService(){
		...
		if (! JitDeliveryInfoServiceClient::$isRegis){
			
			JitDeliveryInfoServiceClient::regis ();
			JitDeliveryInfoServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = JitDeliveryInfoServiceClient::$DEFAULT_PROXY_IP;
		$port = JitDeliveryInfoServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \com\vip\vop\vcloud\jit\JitDeliveryInfoServiceHelper();
	}
