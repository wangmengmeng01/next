			...
			if ("orderPeriodsInfoList" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem4 = new \com\vip\order\common\pojo\order\vo\PrepayOrderPeriodsInfoVO();
						$elem4->read($input);
						
						$this->orderPeriodsInfoList[$_size4++] = $elem4;
					}
					catch(\Exception $e){
