...
class OxoVendorScheduleServiceClient{
	...
	public static function getService(){
		...
		if (! OxoVendorScheduleServiceClient::$isRegis){
			
			OxoVendorScheduleServiceClient::regis ();
			OxoVendorScheduleServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = OxoVendorScheduleServiceClient::$DEFAULT_PROXY_IP;
		$port = OxoVendorScheduleServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \com\vip\vop\schedule\OxoVendorScheduleServiceHelper();
	}
