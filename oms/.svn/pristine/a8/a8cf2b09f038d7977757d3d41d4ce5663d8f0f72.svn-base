...
class wmsDeliveryOrderBatchCreate extends wmsRequest
{
	/**
	 * 创建发货单
	 * @param $params
			    ...
			    if ($response['flag'] == 'success') {
			        if (!empty($params['orders']['order'])) {
			            ...
			            foreach ($params['orders']['order'] as $p_v) {
			                if ($this->checkDeliveryOrder($p_v)) {
			                    $this->insertDeliveryOrder($p_v);
			                }
			            }
			        }
			    	...
			    	$returnMsgArr = $xmlObj->xmlStr2array($returnMsg);
			    	...
			    	if (!empty($returnMsgArr['orders']['order'])) {
			    		...
			    		if (!empty($params['orders']['order']) && empty($params['orders']['order'][0])) {
		    				$params['orders']['order'] = array($params['orders']['order']);
			    		}
			    		
			    		$rsOrderNum = count($returnMsgArr['orders']['order']);
			    		$reqOrderNum = count($params['orders']['order']);
			    		
			    		if ($rsOrderNum != $reqOrderNum) {//部分订单失败
			    			$reqOrders = $params['orders']['order'];
			    			foreach ($returnMsgArr['orders']['order'] as $rsOrder) {
			    				foreach ($reqOrders as $reqKey=>$reqOrder) {
			    					if ($rsOrder['deliveryOrderCode'] == $reqOrder['deliveryOrder']['deliveryOrderCode']) {
			    						unset($reqOrders[$reqKey]);
			    					}
			    				}
			    			}
			    			
			    			if (!empty($reqOrders)) {
			    				foreach ($reqOrders as $succOrder) {
			    					if ($this->checkDeliveryOrder($succOrder)) {
			    						$this->insertDeliveryOrder($succOrder);
			    					}
			    				}
			    			}
			    		} 
	...
	public function checkDeliveryOrder($params)
	{
		...
		$customerId = empty($params['orderLines']['orderLine'][0]['ownerCode']) ? qimen_service::$_customerId :$params['orderLines']['orderLine'][0]['ownerCode'] ;
	...
	public function insertDeliveryOrder($params)
	{
		...
		$values[':customer_id'] = empty($params['orderLines']['orderLine'][0]['ownerCode']) ? (empty($params['orderLines']['orderLine']['ownerCode']) ? : $params['orderLines']['orderLine']['ownerCode']): $params['orderLines']['orderLine'][0]['ownerCode'];
