			...
			if ("related_categories" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$this->related_categories[$_size0++] = $elem0;
					}
					catch(\Exception $e){
			...
			if ("mapping" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem2 = new \vipapis\category\CategoryMapping();
						$elem2->read($input);
			...
			if ("salve_parent_category_ids" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$this->salve_parent_category_ids[$_size3++] = $elem3;
					}
					catch(\Exception $e){
			...
			if ("attributes" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem4 = new \vipapis\category\Attribute();
						$elem4->read($input);
