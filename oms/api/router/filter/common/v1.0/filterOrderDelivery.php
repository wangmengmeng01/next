 ...
 *
 */
class filterOrderDelivery extends msg
{
	...
	public function create(&$requestData)
	{
		...
		foreach ($headers as $k => $v)
		{
			...
			//校验收货人ID
			if (empty($v['ConsigneeID'])) {
				$error_arr[$k] = $this->get_error_data($k, 'S003', '下单平台（收货人ID）不能为空', $v, $error_arr);
				continue;
			}
			//校验订单下发方
			...
			//如果是销售出库校验订单的收货人姓名、电话和地址
			if ($v['OrderType'] == 'SO') {
				if (empty($v['ConsigneeName'])) {
					$error_arr[$k] = $this->get_error_data($k, 'S003', '销售出库单中收货人名称不能为空', $v, $error_arr);
					continue;
				}
				if (empty($v['C_Tel1']) && empty($v['C_Tel2'])) {
					$error_arr[$k] = $this->get_error_data($k, 'S003', '销售出库单中手机号和固话不能都为空', $v, $error_arr);
					continue;
				}else {
				    if (empty($v['C_Tel1'])) {
				        $v['C_Tel1'] = $v['C_Tel2'];
				...
				if (empty($v['C_Address1'])) {
					$error_arr[$k] = $this->get_error_data($k, 'S003', '销售出库单中地址1不能为空', $v, $error_arr);
					continue;
				}
			}
			//明细信息校验
				...
				foreach ($v['detailsItem'] as $val)
				{
					...
					if (empty($val['SKU'])) {
						...
						if (!in_array($val['SKU'], $skuArr) && STRICT_VERIFY_FLAG) {
							...
							//校验SKU是否有库存
// 							$sql = 'SELECT SKU,qty FROM t_product_inventory WHERE customer_id=:customer_id AND warehouse_code=:warehouse_code AND sku=:sku';
// 							$model = $db->prepare($sql);
// 							$model->bindParam(':customer_id', $v['CustomerID']);
// 							$model->bindParam(':warehouse_code', $v['WarehouseID']);
// 							$model->bindParam(':sku', $val['SKU']);
...
// 							$rs = $model->fetch(PDO::FETCH_ASSOC);
// 							if (empty($rs)) {
// 								$error_arr[$k] = $this->get_error_data($k, 'S003', '订单明细中SKU:'.$val['SKU'].'在'.$v['WarehouseID'].'仓库没有库存', $v, $error_arr);
// 								$m++;
// 								break;
// 							}
							$skuArr[$k] = $val['SKU'];
