 ...
 *
 */
class filterCancelSo extends msg
{
	...
	public function create(&$requestData)
	{
		...
		foreach ($headers as $k => $v)
		{
			...
			if (empty($v['OrderType'])) {
				...
				continue;
			} elseif (!in_array($v['OrderType'], array('SO', 'TO', 'RP', 'IL', 'OO'))) {
			...
			if (empty($v['OrderNo'])) {
				...
				if ($exists_arr[$v['WarehouseID']][$v['OrderType']][$v['OrderNo']] != '') {
					$error_arr[$k] = $this->get_error_data($k, 'S006', '订单数据重复', $v, $error_arr);
					continue;
				}			
				//验证订单号是否存在	
				...
				$rs = $model->fetch(PDO::FETCH_ASSOC);
				if(empty($rs)) {
					$error_arr[$k] = $this->get_error_data($k, 'S006', '该出库单号不存在', $v, $error_arr);
					continue;
				} elseif ($rs['order_status'] == '90') {
					$error_arr[$k] = $this->get_error_data($k, 'S006', '该出库单号状态为已经取消', $v, $error_arr);
					continue;
				}	
			}
