...
class wmsRequest
{
    ...
    public function send()
    {
    	...
    	if (CHECK_TIANCAN == 1){
            ...
            if ($url == '') {
                ...
                return $this->msgObj->outputQimen('failure', $url_msg, 'S002', $logExt);
            }

			$rs = $httpObj->post($url, $apiParams);
			qimen_service::$_mid_resp_time = util::microtime_float();
    ...
    public function getApiUrl(){
        global $db;
        $sql = "SELECT wms_url FROM t_base_warehouse WHERE warehouse_code=:warehouse_code;";
    ...
    public function get_dataBase_relation($type)
    {
    	...
    	if ($type == 'DeliveryOrderCreateInfo') {
    	...
    	} elseif ($type == 'entry_order_create_related_order') {
    	    $return_arr['orderType'] = 'order_type';
    	...
    	} elseif ($type == 'stock_out_related_order_info') {
    	    $return_arr['orderType'] = 'order_type';
    	    $return_arr['orderCode'] = 'order_code';
    	    ...
    	    $return_arr['customer_id'] = 'customer_id';
    	    $return_arr['sourcePlatformCode'] = 'udf1';
    	    $return_arr['shopCode'] = 'shop_code';
    	    $return_arr['shopName'] = 'descr_c';
    	    $return_arr['platformShopCode'] = 'udf2';
    	    $return_arr['platformShopName'] = 'udf3';
    	    $return_arr['zipCode'] = 'zip';
    	    $return_arr['province'] = 'shop_province';
    	    $return_arr['city'] = 'shop_city';
    	    $return_arr['area'] = 'shop_area';
    	    $return_arr['town'] = 'shop_town';
