...
class erpDeliveryOrderConfirm extends erpRequest
{
	/**
	 * 推送erp发货单确认信息
	 * @param $params
		...
		if (empty($params)) {
			...
			if (!empty($response)) {
				if ($response['flag'] == 'success') {
					//写入数据库
					$this->insertConfirmOrder($params);
					//返回
	...
	public function insertConfirmOrder($params)
	{
		...
		//查询出deliveryId
		$sql = "SELECT delivery_id,delivery_order_code FROM t_delivery_order_info WHERE customer_id = :customer_id AND warehouse_code = :warehouse_code AND delivery_order_code = :delivery_order_code";
		...
		$deliveryOrderCode = $rs['delivery_order_code'];
		
		//更新缺货订单处理状态为已处理
		$updateShortageOrderStatusSql = "UPDATE t_delivery_order_shortage SET done_flag = '2' WHERE delivery_order_code=:delivery_order_code AND customer_id = :customer_id";
		$updateShortageOrderStatusModel = $db->prepare($updateShortageOrderStatusSql);
		$updateShortageOrderStatusModel->bindParam(':delivery_order_code',$deliveryOrderCode);
		$updateShortageOrderStatusModel->bindParam(':customer_id',qimen_service::$_customerId);
		$updateShortageOrderStatusModel->execute();
