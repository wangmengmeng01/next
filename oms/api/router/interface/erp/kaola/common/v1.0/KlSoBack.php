...
class KlSoBack extends erpRequest
{
    /**
     * 请求并根据返回处理数据
     * @param $params 请求数据
    ...
    public function insertBackInfo($params)
    {
        ...
        $orderInfo = OmsDatabase::$oms_db->fetchOne('order_id','t_outbound_info',$fWhere,$fParam);

        if (isset($orderInfo['order_id'])) {
            //插入单头信息
            $addData = array(
                'order_id'=>$orderInfo['order_id'],
                'oms_order_no'=>$params['outbound_id'],
                'order_type'=>$params['type'],
                'customer_id'=>kaola_service::$_ownerId,
                'warehouse_code'=>kaola_service::$_stockId,
                'create_time'=>date("Y-m-d H:i:s")
            );
            $recordId = OmsDatabase::$oms_db->insert('t_outbound_info_record',$addData);
            ...
            if (!empty($params['items'])) {
                ...
                foreach ($params['items'] as $iKey=>$item) {
                    ...
                    $itemsData[$iKey]['order_no'] = $params['outbound_id'];
                    $itemsData[$iKey]['sku'] = $item['sku_id'];
                    $itemsData[$iKey]['qty_shipped'] = $item['good_qty'];
                    $itemsData[$iKey]['create_time'] = date("Y-m-d H:i:s");
                }
                if (!empty($itemsData)) {
                    OmsDatabase::$oms_db->insertAll('t_outbound_detail_record',$itemsData);
                }
            }
        }
    }
