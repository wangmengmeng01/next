...
class cainiaoRequest {
            ...
            if (!empty($sellerInfo)) {
                if (!empty($sellerInfo['seller_id']) && !empty($sellerInfo['platform_elec'])) {
                    ...
                    if (!empty($tokenInfo)) {
                        ...
                        //获取地址信息
                        $findAddrSql = "SELECT ship_prov,ship_city,ship_county,ship_town,ship_detail_address FROM csk_seller_waybill_info WHERE seller_id = :seller_id AND ship_addr_code = :ship_addr_code";
    ...
    public function getTokenBySellerId ($params) {
        ...
        if (!empty($accessTokenInfo)) {
            ...
            if (cainiao_service::$_method != 'taobao.wlb.waybill.i.print') {//为了RYD的get接口
                $infoArr['ship_addr_info']['ship_prov'] = $params['shipping_address']['province'];
                $infoArr['ship_addr_info']['ship_city'] = empty($params['shipping_address']['city']) ? '' : $params['shipping_address']['city'];
                $infoArr['ship_addr_info']['ship_county'] = empty($params['shipping_address']['area']) ? '' : $params['shipping_address']['area'];
                $infoArr['ship_addr_info']['ship_town'] = empty($params['shipping_address']['town']) ? '' : $params['shipping_address']['town'];
                $infoArr['ship_addr_info']['ship_detail_address'] = $params['shipping_address']['address_detail'];
            }
            return $infoArr;
        } else {
    ...
    public function get_database_relation ($cname) {
        ...
        if ($cname == 'search_addr') {
            ...
            $returnArr['town'] = 'ship_town';
            $returnArr['detail'] = 'ship_detail_address';
        } elseif ($cname == 'jd_seller_authorization'){
            $returnArr['seller_id'] = 'plat_sales_code';
            $returnArr['access_token'] = 'accessToken';
        }
        return $returnArr;
    }
