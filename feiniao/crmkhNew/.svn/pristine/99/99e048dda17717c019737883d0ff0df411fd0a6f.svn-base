var prefix = "/crmkh/warning/warningBranchMobile";
 ...
 */
function query(){
	var branchCode=$('#searchBranchCode').val(),
    	status=$('#searchStatus').val();
...
function exportExcel(action) {
	...
	$('#exportExcelbtn').attr("readonly","readonly")//将input元素设置为readonly 
	  ...
	  var url = prefix + action+"?branchCode="+$('#searchBranchCode').val()+"&status="+$('#searchStatus').val();
	  ...
	  xhr.onload = function () {
	    ...
	    if (this.status === 200) {
	      ...
	      reader.onload = function (e) {
	        ...
	        a.download = '预警短信网点手机号信息表.xlsx';//导出表的表名   自行修改
