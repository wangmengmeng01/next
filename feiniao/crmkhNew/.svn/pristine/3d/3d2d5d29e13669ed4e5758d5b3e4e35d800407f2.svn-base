...
public class ReportFluctuateServiceImpl implements ReportFluctuateService {
	...
	private List<ReportFluctuateDO> doWithCustnumber( Bo_ReportFluctuate boReportFluctuate,String startDate,String endDate, String type) throws ParseException {
		...
		for (ReportFluctuateDO custData : custDatas) {
			...
			if("拼多多".equals(custData.getCustomerSourceType())){
				String customerId = custData.getCustomerId()+"pdd";
				Map<String, Object> sellerPDDInfo = operationsCqkh.get(customerId);
				if(sellerPDDInfo != null && sellerPDDInfo.get("vendor_code") != null) {
					custData.setSellerId(sellerPDDInfo.get("vendor_code").toString());
					custData.setSellerName(sellerPDDInfo.get("vendor_name").toString());
				}
			}
