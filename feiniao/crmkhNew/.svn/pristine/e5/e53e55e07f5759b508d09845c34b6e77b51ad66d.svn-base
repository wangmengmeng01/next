...
public class CostreportCustRouteIncomeServiceImpl implements CostreportCustRouteIncomeService {
	...
	public List<CostreportCustRouteIncomeDO> getIncomelist(
		...
		for(CostreportCustRouteIncomeDO data : dataList){
			...
			if("5".equals(data.getCustomerSourceType())){
				String customerId = data.getCustomerId()+"pdd";
				Map<String, Object> sellerPDDInfo = operationsCqkh.get(customerId);
				if(sellerPDDInfo != null && sellerPDDInfo.get("vendor_code") != null) {
					data.setSellerId(sellerPDDInfo.get("vendor_code").toString());
					data.setSellerName(sellerPDDInfo.get("vendor_name").toString());
				}
			}
