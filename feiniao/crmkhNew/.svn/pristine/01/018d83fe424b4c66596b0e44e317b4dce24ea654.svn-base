...
public class WarningHandleDO implements Serializable {
	...
	public String getShowBranchDealType() {
		 if("A".equals(branchDealType)){
			 showBranchDealType =  "A同行竞争(因价格、服务质量不足等造成客户流失)";
	   	 }else if("B".equals(branchDealType)){
	   		showBranchDealType =  "B客户原因(客户无货、被封店、等)";
	   	 }else if("C".equals(branchDealType)){
	   		showBranchDealType =  "C促销影响(客户促销后的短期业务量不稳定)";
	   	 }else if("D".equals(branchDealType)){
	   		showBranchDealType =  "D称重问题(称重导致流失)";
	   	 }else if("E".equals(branchDealType)){
	   		showBranchDealType =  "E其他(季节性、海关政策等)";
	   	 }
