...
public class ReportTotaldataDO implements Serializable {
	...
	public ReportTotaldataDO toReckonList() {
	   ...
	   double customerSum = aCustomerSum+bCustomerSum+cCustomerSum+dCustomerSum+eCustomerSum+fCustomerSum+gCustomerSum;//电子面单客户总数
	   
		 totalData.setOrderSum(orderSum);//全网总单量
		 totalData.setOrderAvg(Double.parseDouble(df.format(orderAvg)));//全网平均单量
		 
		 totalData.setDianziOrderSum(dianziOrderSum);//电子面单总量
		 totalData.setOrdinaryOrderSum(ordinaryOrderSum);//普通面单总量
		 totalData.setDianziPercent(orderSum>0?nt.format(dianziOrderSum/orderSum):"0.0%");//电子面单占比
		 
		 totalData.setCustomerSum(customerSum); //电子面单客户数
		 //totalData.setCustomerAvgSum(Double.parseDouble(df.format(customerAvgSum)));
		 totalData.setDianziOrderSumAvg(Double.parseDouble(df.format(dianziOrderSumAvg)));//电子面单总量日均单量
		 //totalData.setCustomerPriceSum(Double.parseDouble(dfDouble.format(customerPriceSum)));
		 totalData.setDianziPriceSumAvg(Double.parseDouble(df.format(dianziPriceSumAvg)));//电子面单日均奖励金额
		 //totalData.setCustomerAllPriceSum(Double.parseDouble(dfDouble.format(customerAllPriceSum)));
		 totalData.setDianziPriceSum(Double.parseDouble(df.format(dianziPriceSum)));//电子面单总奖励金额
		 ...
		 totalData.setACustomerSum(aCustomerSum);//A类 客户数
		 totalData.setAOrderAvg(Double.parseDouble(df.format(aOrderAvg)));//A类电子面单 日均单量
		 totalData.setAOrderSum(aOrderSum);//A类电子面单 总单量
		 totalData.setAPricePercent(orderSum>0?nt.format(aOrderSum/orderSum):"0.0%");//A类电子面单总量/全网总单量
		 totalData.setAPriceSum(Double.parseDouble(df.format(aPriceSum)));//A类电子面单 日均奖励金额
		 totalData.setAAllPriceSum(Double.parseDouble(df.format(aAllPriceSum)));//A类电子面单 总奖励金额
