 ...
 */
public class MarketKeyProvinceReportDO implements Serializable {
	...
	private Double proportionYd = (double) 0;
	//圆通占有率
	@ExcelField(title = "圆通占有率", order = 15)
	private Double proportionYt = (double) 0;
	//中通占有率
	@ExcelField(title = "中通占有率", order = 16)
	private Double proportionZt= (double) 0;
	//申通占有率
	@ExcelField(title = "申通占有率", order = 17)
	private Double proportionSt= (double) 0;
	//百世占有率
	@ExcelField(title = "百世占有率", order = 18)
	private Double proportionBs= (double) 0;
	//审核人
	...
	public MarketKeyProvinceReportDO toDataHandle(Bo_marketKeyProvince boMarketKeyProvince) {
		MarketKeyProvinceReportDO marketKeyProvinceReportDO = new MarketKeyProvinceReportDO();
		DecimalFormat    df   = new DecimalFormat("######0.00"); 
		 NumberFormat nt = NumberFormat.getPercentInstance();
	   //设置百分数精确度2即保留两位小数
	   nt.setMinimumFractionDigits(2);
	   //韵达全国月总量
    double ydyzlhj = boMarketKeyProvince.getYdyzl();
    //快递行业月总量
    double kdhyzlhj = boMarketKeyProvince.getKdhyzl();
    
    double skdhyzl = 0;
    double skdhyrj = 0;
    //各省快递行业总量
    if(proportionYd != 0.0){
    	//proportionYd 此处表示韵达该省业务总量在该省快递行业月总量中的占比
    	//skdhyzl 省快递行业月总量
    ...
    if(ydyzlhj != 0.0 && kdhyzlhj != 0.0){
    	 	...
    	 	marketKeyProvinceReportDO.setResponsiblePeople(responsiblePeople);
    	    marketKeyProvinceReportDO.setMonthScore(monthScore);
    	    
    	    AB = nt.format(orderSum/ydyzlhj);
    	    marketKeyProvinceReportDO.setAB(AB);
    	    
    	    CD = nt.format(skdhyzl/kdhyzlhj);
    	    marketKeyProvinceReportDO.setCD(CD);
    	    
    	    kdqsrjdl = df.format(skdhyrj/10000);
    	    marketKeyProvinceReportDO.setKdqsrjdl(kdqsrjdl);
    	    
    	    kdqsyzl = df.format(skdhyzl/10000);
    	    marketKeyProvinceReportDO.setKdqsyzl(kdqsyzl);
    	    
    	    ydqsyzl = df.format(orderSum/10000);
    	    marketKeyProvinceReportDO.setYdqsyzl(ydqsyzl);
    	    
    	    ydqsrjl = df.format(orderAvg/10000);
    	    marketKeyProvinceReportDO.setYdqsrjl(ydqsrjl);
    	    
    	    ydqsscfezb = nt.format(proportionYd/100);
    	    marketKeyProvinceReportDO.setYdqsscfezb(ydqsscfezb);
    	    ...
    	    marketKeyProvinceReportDO.setZtqsyzl(ztqsyzl);
    	    
    	    ztqsrjl = df.format((skdhyrj*(proportionZt/100))/10000);
    	    marketKeyProvinceReportDO.setZtqsrjl(ztqsrjl);
    	    
    	    ztqsscfezb = nt.format(proportionZt/100);
    	    marketKeyProvinceReportDO.setZtqsscfezb(ztqsscfezb);
    	    ...
    	    marketKeyProvinceReportDO.setYtqsyzl(ytqsyzl);
    	    
    	    ytqsrjl = df.format((skdhyrj*(proportionYt/100))/10000);
    	    marketKeyProvinceReportDO.setYtqsrjl(ytqsrjl);
    	    
    	    ytqsscfezb = nt.format(proportionYt/100);
    	    marketKeyProvinceReportDO.setYtqsscfezb(ytqsscfezb);
    	    ...
    	    marketKeyProvinceReportDO.setStqsyzl(stqsyzl);
    	    
    	    stqsrjl = df.format((skdhyrj*(proportionSt/100))/10000);
    	    marketKeyProvinceReportDO.setStqsrjl(stqsrjl);
    	    
    	    stqsscfezb = nt.format(proportionSt/100);
    	    marketKeyProvinceReportDO.setStqsscfezb(stqsscfezb);
    	    ...
    	    marketKeyProvinceReportDO.setBsqsyzl(bsqsyzl);
    	    
    	    bsqsrjl = df.format((skdhyrj*(proportionBs/100))/10000);
    	    marketKeyProvinceReportDO.setBsqsrjl(bsqsrjl);
    	    
    	    bsqsscfezb = nt.format(proportionBs/100);
    	    marketKeyProvinceReportDO.setBsqsscfezb(bsqsscfezb);
	...
	return marketKeyProvinceReportDO;
