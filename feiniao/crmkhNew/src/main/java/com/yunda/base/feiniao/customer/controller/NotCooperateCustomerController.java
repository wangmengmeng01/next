...
public class NotCooperateCustomerController extends BaseController {
	...
	public PageUtils list(@ModelAttribute NotCooperateCustomerBO notCooperateCustomerBO, HttpServletRequest request){
		...
		if (loginUser.hasPerms(Constant.REPORT_ADMIN_PERMS)) {
			...
			if(loginUser.isBigareaqx()){
				List<String> regionIdsQX = loginUser.getBigareaNames();
				List<String> listProvinceName = notCooperateCustomerService.getProvinceNameByBigareaNames(regionIdsQX);
				
				notCooperateCustomerBO.setProvinceNamesQX(listProvinceName);//省名称控制数据权限  省总可能有多个省
                ...
                notCooperateCustomerList = notCooperateCustomerService.listInfoByProvinceName(notCooperateCustomerBO);
                total = notCooperateCustomerService.countByProvinceName(notCooperateCustomerBO);
			} else if (StringUtils.isNotBlank(loginUser.getOrgCode())) {// 只有某网点权限
	...
	public int dealBefore(@PathVariable("id") Integer id,HttpServletRequest request){
		...
		if (loginUser.hasPerms(Constant.REPORT_ADMIN_PERMS)) {
			//仅总部待处理才可以允许总部处理
			if(!state.equals(Constant.ZONGBU_WAIT_DEAL)&&!state.equals(Constant.NOT_COOPERATE)){
				return 300;
			}
		} else {
	...
	public R dealSave(NotCooperateCustomerDO notCooperateCustomer){
		if("不属实".equals(notCooperateCustomer.getFeedBackCheck())){
        ...
        //校验合作网点编码是否正确
		//先查询所填写的合作网点编码是否存在
		if(null!=notCooperateCustomer.getCooperateBranch() && !notCooperateCustomer.getCooperateBranch().equals("")){
			int count = notCooperateCustomerService.checkCooperateBranch(notCooperateCustomer.getCooperateBranch());
			if(count<1){
				return R.error("合作网点编码不存在!");
			}
		}
		/*if(null!=notCooperateCustomer.getProvinceVisitTime() && !notCooperateCustomer.getProvinceVisitTime().equals("")&&notCooperateCustomer.getProvinceVisitTime().length()>=9){
			String str = notCooperateCustomer.getProvinceVisitTime().substring(0, 10);
			notCooperateCustomer.setProvinceVisitTime(str);
		}else{
			notCooperateCustomer.setProvinceVisitTime("");
		}*/
		//根据id将处理的结果保存
		int update = notCooperateCustomerService.update(notCooperateCustomer);
		if(update<1){
